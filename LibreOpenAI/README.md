...
```cs
    ...
    JToken result = await openAi.Chat.Completions.CreateJToken(request);
...
```cs
    ...
    string firstPartOfResult = ((JValue)result [1]["choices"][0]["delta"]["content"]).Value;
```
###### Using JSON
```cs
    string request = "{\"max_completion_tokens\":50,\"model\":\"gpt-4o\",\"messages\":[{\"role\":\"user\",\"content\":\"What is the capital of France?\"}],\"n\":1}";
